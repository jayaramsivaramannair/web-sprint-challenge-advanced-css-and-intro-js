...
for(let element of artists) {
  if(element.name === "Vincent van Dough") {
      element.name = "Vincent Van Gogh";
  }
}

for(let element of artists) {
  if(element.name === "Vincent Van Gogh") {
      console.log(element);
  }
}
...
function get20s(artArray){
  let artistTwentyCentury = [];
    for(let element of artArray) {
        let yearsActive = element.years.split(" ");
        let yearBirth = parseInt(yearsActive[0]);
        let yearDeath = parseInt(yearsActive[2]);
        if(yearBirth >= 1900 && yearDeath <= 2000) {
            artistTwentyCentury.push(element.name);
        }
    }
    return artistTwentyCentury;
}
...
function lotsOfArt(artArray){
  ...
  for(let element of artArray) {
    if(element.paintings > 100) {
      moreThanHundred.push(element.name);
    }
  }
